...
import { EmployeeModel, PageRequest, PageResponse } from '../../interfaces/models/Api';
...
const Employees: FC = () => {
	...
	const [response, fetchEmployees, isPending] = useFetchData<PageResponse<EmployeeModel>>(`/employees`, {
		params
	});
	...
	if (employees?.length === 0) {
		return (
			<div className={`d-flex mh-100 w-50 justify-content-center align-items-center p-2 bg-dark rounded-card-10`}>
				<span className={'fw-bold fs-3 text-light text-center w-50'}>Employees not found. Try again later.</span>
			</div>
		);
	}
	...
	return (
		<div className={` w-100 h-90 align-self-center d-flex flex-column pb-3`}>
			...
			>
				<h3 className={'text-light text-center pt-1 bg-secondary-dark py-1 px-2 rounded-card-10'}>Employees</h3>

				<Row className={'row-gap-3 pb-4 w-100'}>
					{employees?.map((employee, k) => (
						<Col key={k} xs={24} sm={12} md={6} xxl={4}>
							<EmployeeCard employee={employee} fetchEmployees={fetchEmployees} />
						</Col>
					))}
				</Row>
			</div>
